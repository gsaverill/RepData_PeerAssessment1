...
data_1$day_of_week = as.factor(data_1$day_of_week)
```

Lastly, we'll create a new factor variable in the dataset with two 
...
data_1$day_type = as.factor(data_1$day_type)
```
...
data_1$day_type <- 
    factor(data_1$day_type, levels = new_order)
...
#**`r interval_with_max_number_of_average_steps`** with 
...
Find the average number of steps for each interval across all weekdays and
